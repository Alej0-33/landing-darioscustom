...
import { i18n } from "@/i18n.config"; // <-- IMPORTACIÓN AÑADIDA
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: "/", languages: { "en-US": "/en", "es-US": "/es" } },
    openGraph: { title: dict.seo.title, description: dict.seo.description, url: SITE_URL, siteName: "Darioscustom", images: [{ url: "/brand/darioscustom_logo.webp", width: 800, height: 600 }] },
  };
}
    ...
    <html lang={resolvedParams.lang} className={cn("dark scroll-smooth", montserrat.variable, "font-sans", geist.variable)}>
      <body className="bg-[#09090B] text-[#FFFFFF] antialiased min-h-screen selection:bg-[#B85227] selection:text-white">
        <DictionaryProvider dictionary={dict}>
          <a href="#inicio" className="sr-only focus:not-sr-only focus:absolute focus:z-50 focus:p-4 focus:bg-[#B85227] focus:text-white">
            Saltar al contenido
          </a>
          {children}
        </DictionaryProvider>
